...
def run_simulation(model, env, model_name, episodes=15000, max_steps=400):
    rewards = []
    steps_to_goal = []
    goals_reached = 0
...
if __name__ == "__main__":
    ...
    ardns_rewards, ardns_steps, ardns_goals = run_simulation(ardns_model, env, "ARDNS-P", episodes=15000)
    
    # Reset environment for DQN
    np.random.seed(42)  # Reset seed for consistency
    env = GridWorld(size=10)
    print("\nStarting DQN simulation...")
    dqn_rewards, dqn_steps, dqn_goals = run_simulation(dqn_model, env, "DQN", episodes=15000)
    
    # Print results for ARDNS-P
    print("\nARDNS-P Final Results:")
    print(f"Goals reached: {ardns_goals}/15000 ({ardns_goals/15000*100:.1f}%)")
    ...
    print("\nDQN Final Results:")
    print(f"Goals reached: {dqn_goals}/15000 ({dqn_goals/15000*100:.1f}%)")
